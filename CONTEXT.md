...
Two Managed Agents deployment: a **Support Agent** that answers employee queries from a memory store of policy files via progressive disclosure, and an **Agent Improver** that analyzes the Support Agent's past sessions and proposes changes to policy files or the Support Agent's system prompt.
...
_Avoid_: Improver (bare, without "Agent"), review agent, meta-agent

**Suggestion**:
One aggregated, evidenced recommendation produced by the Agent Improver — targets either a Policy File or the Support Agent's system prompt, and cites the session(s) that support it. Multiple sessions sharing the same root cause collapse into a single Suggestion.
...
- Sessions API supports `list` and per-session `retrieve`, with no bulk/batch export — the Agent Improver must list then retrieve sessions one at a time.
